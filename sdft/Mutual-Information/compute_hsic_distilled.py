...
def main():
    ...
    file_sd = os.path.join(base, "predictions", ds, "sdft", "inference_distilled_sdft", "generated_predictions.jsonl")
    file_sf = os.path.join(base, "predictions", ds, "sft",  "inference_distilled_sft",  "generated_predictions.jsonl")

    hsic_sd = compute_hsic_for_file(file_sd,  f"{ds} Distilled↔SD‑FT")
    hsic_sf = compute_hsic_for_file(file_sf,  f"{ds} Distilled↔SFT")

    results_dir = ensure_results_dir(ds)
    log_path = os.path.join(results_dir, f"{ds}_cross_hsic.log")
    with open(log_path, "w") as fout:
        fout.write(f"Dataset: {ds}\n")
        fout.write(f"Distilled_vs_SD-FT_HSIC: {hsic_sd:.6f}\n")
        fout.write(f"Distilled_vs_SFT_HSIC:   {hsic_sf:.6f}\n")
