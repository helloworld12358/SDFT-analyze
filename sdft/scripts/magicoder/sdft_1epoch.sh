...
output_folder="epoch1_predictions/${train_dataset}/${type}"
result_file="epoch1_results/${train_dataset}/${type}.log"
checkpoint_dir="epoch1_checkpoints/${train_dataset}/${type}"
tmp_predictions_dir="epoch1_predictions/${train_dataset}/${type}/distilled"
...
# Export environment variables for multi-GPU setup
export CUDA_VISIBLE_DEVICES=${cuda_visible_devices}
export NCCL_DEBUG=INFO
export NCCL_P2P_LEVEL=NVL

# Generate distilled dataset using distributed prediction
torchrun \
  --standalone \
  --nproc_per_node=4 \
  --master_port=12345 \
...
# Train on distilled dataset using distributed training
torchrun \
  --standalone \
  --nproc_per_node=4 \
  --master_port=12346 \
