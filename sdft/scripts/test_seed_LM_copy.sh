...
do
    ...
    torchrun --standalone --nproc_per_node=4 --master_port=12345 main.py \
        --stage sft \
...
torchrun --standalone --nproc_per_node=4 --master_port=12346 main.py \
    --stage sft \
    --model_name_or_path ${model_path} \
    --do_predict \
...
# Evaluate on HumanEval (single-process)
...
torchrun --standalone --nproc_per_node=4 --master_port=12348 main.py \
    --stage sft \
    --model_name_or_path ${model_path} \
    --do_predict \
    --dataset alpaca_eval \
...
do
    ...
    torchrun --standalone --nproc_per_node=4 --master_port=12349 main.py \
        --stage sft \
        --model_name_or_path ${model_path} \
        --do_predict \
        --dataset advbench \
