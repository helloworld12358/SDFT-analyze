...
# 准备输出目录（以 _base 后缀区分，不覆盖已有 adapter 分析）
rm -rf "${OUTPUT_FOLDER}"
...
if [[ -n "${ADAPTER_DIR}" ]]; then
  ...
  echo "[Info] No adapter specified — analyze script will auto-inject lora_target=q_proj,v_proj and create in-memory LoRA for analysis."
fi
