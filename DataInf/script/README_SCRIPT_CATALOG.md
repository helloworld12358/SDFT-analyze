...
| `loss_theory_01_forward_collect.py` | Forward-only collector for per-sample/token loss diagnostics (epoch_1/epoch_5). | Writes `sample_stats.csv`, `token_subsample_stats.csv`, `token_sequence_probe_stats.csv` (+parquet when available), with resume state files under `results/loss_theory/by_combo/...`. | No backward/grad/Hessian; supports streaming, batching, shard split, and resume. |
| `loss_theory_02..09_*.py` | Theory diagnostics pipeline: tail shape, MGF check, empirical Bernstein coverage, robust means, conditional analysis, length ablation, dependence, final report. | Writes outputs under `results/loss_theory/analysis/...` including plots/csv/json and final markdown report. | Consumes outputs from `loss_theory_01_forward_collect.py`. |
| `run_loss_theory_collect_shard.sh` | Launcher for sharded forward collection, designed for multi-machine runs. | Uses `LOSS_THEORY_SHARD_COUNT/LOSS_THEORY_SHARD_INDEX` plus local multi-GPU split. | Run on each worker machine (recommended 7-way sharding). |
| `run_loss_theory_analyze.sh` | Sequential launcher for analysis scripts (`02` to `09`). | Produces final report JSON/MD under `results/loss_theory/analysis/final_report`. | Run once after collection is complete. |
