...
# 每个组合调用 compute_pairwise_from_grads_tagged_localresults.py 生成矩阵并写入 txt（内积，默认不 normalize）

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DATAINF_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
PYTHON="python"
PY_SCRIPT="${DATAINF_ROOT}/src/compute_pairwise_from_grads_tagged_localresults.py"
