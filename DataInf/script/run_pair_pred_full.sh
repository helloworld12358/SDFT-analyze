...
PAIR_PRED_EPOCH="${PAIR_PRED_EPOCH:-epoch_1}"
...
"$PYTHON_BIN" "$SCRIPT_DIR/pair_pred_01_extract_pair_features.py" --datainf_root "$DATAINF_ROOT"
"$PYTHON_BIN" "$SCRIPT_DIR/pair_pred_02_score_predictor.py" \
  --datainf_root "$DATAINF_ROOT" \
  --epoch "$PAIR_PRED_EPOCH" \
  --mode "$PAIR_PRED_MODE" \
  --alpha "$PAIR_PRED_ALPHA" \
  --beta "$PAIR_PRED_BETA" \
  --gamma "$PAIR_PRED_GAMMA" \
  --grid_values "$PAIR_PRED_GRID_VALUES"
"$PYTHON_BIN" "$SCRIPT_DIR/pair_pred_03_final_summary.py" --datainf_root "$DATAINF_ROOT" --epoch "$PAIR_PRED_EPOCH"
