...
Extract pair-wise features for (train_dataset=A, feature_epoch=ef, label_epoch=el, test_task=B):
- Cent
- Load
- Self
...
def main() -> None:
    ...
    # --epoch / --all_epochs are treated as feature-epoch selectors.
    ...
    p.add_argument("--all_feature_epochs", action="store_true")
    p.add_argument("--label_epoch", type=str, default="", help="If empty, use same as feature_epoch")
    p.add_argument("--all_label_epochs", action="store_true", help="Use all label epochs for each feature epoch")
    ...
    if args.all_feature_epochs or args.all_epochs:
        feature_epochs = list(DEFAULT_EPOCHS)
    elif args.feature_epoch.strip():
        feature_epochs = _pick_list(args.feature_epoch, DEFAULT_EPOCHS)
    elif args.epoch.strip():
        feature_epochs = _pick_list(args.epoch, DEFAULT_EPOCHS)
    else:
        feature_epochs = list(DEFAULT_EPOCHS)

    if args.all_label_epochs:
        label_epochs: List[str] = list(DEFAULT_EPOCHS)
    elif args.label_epoch.strip():
        label_epochs = _pick_list(args.label_epoch, DEFAULT_EPOCHS)
    else:
        label_epochs = []
    ...
    for ds in train_datasets:
        for feature_epoch in feature_epochs:
            existing = load_existing_ownh_from_analysis(result_roots, ds, feature_epoch)
            ...
            if not (_is_5x5(T_sft) and _is_5x5(C_sft) and _is_5x5(T_sdft) and _is_5x5(C_sdft)):
                reason = write_unavailable_note(
                    os.path.join(unavailable_dir, f"unavailable_{ds}_{feature_epoch}_matrix.json"),
                ...
                unavailable_rows.append({"train_dataset": ds, "feature_epoch": feature_epoch, "reason_file": os.path.abspath(reason)})
                continue

            label_epochs_used = label_epochs if label_epochs else [feature_epoch]
            for label_epoch in label_epochs_used:
                sft_perf = perf_idx.get((ds, label_epoch, "sft"))
                sdft_perf = perf_idx.get((ds, label_epoch, "sdft"))
                if sft_perf is None or sdft_perf is None:
                    reason = write_unavailable_note(
                        os.path.join(unavailable_dir, f"unavailable_{ds}_feat_{feature_epoch}_label_{label_epoch}_perf.json"),
                    ...
                    unavailable_rows.append(
                        {
                            ...
                            "feature_epoch": feature_epoch,
                            "label_epoch": label_epoch,
                            "reason_file": os.path.abspath(reason),
                        }
                    )
                    continue
                    ...
                    rows.append(
                        {
                            ...
                            "feature_epoch": feature_epoch,
                            "label_epoch": label_epoch,
                            "epoch": feature_epoch,  # backward-compatible field
                            "test_task": task,
                            "task_index": b,
                            "Cent_sft": cent_sft,
                            "Cent_sdft": cent_sdft,
    ...
    for ep in feature_epochs:
        ep_rows = [r for r in rows if str(r.get("feature_epoch", r.get("epoch"))) == ep]
        ep_csv = os.path.join(output_root, f"pair_features_feature_{ep}.csv")
        ep_json = os.path.join(output_root, f"pair_features_feature_{ep}.json")
        write_rows_csv(ep_csv, ep_rows)
        with open(ep_json, "w", encoding="utf-8") as f:
            json.dump(ep_rows, f, ensure_ascii=False, indent=2)

    combo_set = sorted(
        {(str(r.get("feature_epoch", r.get("epoch"))), str(r.get("label_epoch", r.get("true_perf_epoch", r.get("epoch"))))) for r in rows}
    )
    for fe, le in combo_set:
        combo_rows = [
            r
            for r in rows
            if str(r.get("feature_epoch", r.get("epoch"))) == fe
            and str(r.get("label_epoch", r.get("true_perf_epoch", r.get("epoch")))) == le
        ]
        combo_csv = os.path.join(output_root, f"pair_features_feat_{fe}__label_{le}.csv")
        combo_json = os.path.join(output_root, f"pair_features_feat_{fe}__label_{le}.json")
        write_rows_csv(combo_csv, combo_rows)
