...
TARGET_EPOCH="${1:-${SCHEMEA_RECT_H_EPOCH:-all}}"
TARGET_METHOD="${2:-${SCHEMEA_RECT_H_METHOD:-both}}"
TARGET_TRAIN_DATASET="${3:-${SCHEMEA_RECT_H_TRAIN_DATASET:-}}"
USER_EXTRA_ARGS=()
if [ $# -ge 4 ]; then
  USER_EXTRA_ARGS=("${@:4}")
fi
...
echo "[schemeA_rect_hessian] epoch=$TARGET_EPOCH method=$TARGET_METHOD train_dataset=${TARGET_TRAIN_DATASET:-ALL}"
echo "[schemeA_rect_hessian] gpu_ids=$GPU_IDS workers=$PAIR_WORKERS timeout=$PAIR_TIMEOUT"

"$PYTHON_BIN" "$SCRIPT_DIR/schemeA_11_train_test_rect_hessian.py" \
