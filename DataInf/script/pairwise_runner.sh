...
for ((i=0;i<NUM_GRADS;i++)); do
  for ((j=i;j<NUM_GRADS;j++)); do
    out_json="${result_DIR}/sim_${DATASET_NAMES[$i]}_${DATASET_NAMES[$j]}.json"
...
$PYTHON "$ASSEMBLE_PY" --grads_list "$GRADS_LIST_FILE" --result_dir "$result_DIR" --out_csv "$OUT_CSV" --out_npy "$OUT_NPY"
