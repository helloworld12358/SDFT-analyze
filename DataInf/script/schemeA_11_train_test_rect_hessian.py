...
# -*- coding: utf-8 -*-
"""
Scheme A - Step 11 (Hessian, not inverse)
Build train-vs-test rectangular matrices (7x5 style):
- T[A, B] = g_train(A)^T H_A g_test(B)
- C[A, B] = T[A, B] / sqrt( S_train[A] * S_test[A, B] )

Where:
- rows A are train datasets
- cols B are test tasks
- H_A is induced by train dataset A (empirical Hessian proxy from train-sample gradients)
"""
...
def _compute_one_row(
    ...
    calc_script = os.path.join(datainf_root, "src", "calc_dataset_hessian_score.py")
...
def _build_txt_report(path: str, payload: Dict[str, object]) -> str:
    lines: List[str] = []
    lines.append("SchemeA Train-Test Rectangular Matrix Summary (Hessian)")
    lines.append("=" * 88)
...
def main() -> None:
    p = argparse.ArgumentParser(description="Scheme A step11: build train-vs-test rectangular T/C matrices with Hessian.")
    p.add_argument("--datainf_root", type=str, default=None)
    p.add_argument("--output_root", type=str, default=None, help="Default: <result_root>/schemeA/train_test_rect_hessian")
    ...
    output_root = args.output_root or os.path.join(result_root, "schemeA", "train_test_rect_hessian")
    ...
    for e in epochs:
        for m in methods:
            ...
            tag = f"train_test_hessian_{e}_{m}_7x5"
        ...
        if "sft" in methods and "sdft" in methods:
            ...
            tag = f"train_test_hessian_{e}_sft_minus_sdft_7x5"
            ...
            bundle = _save_rect_bundle(
                ...
                col_names=task_names,
                metadata={
                    "mode": "train_test_rect_hessian_diff",
                    "epoch": e,
    ...
    summary_json = os.path.join(output_root, "train_test_rect_hessian_row_summary.json")
    _save_json(summary_json, rows_summary)
    write_rows_csv(os.path.join(output_root, "train_test_rect_hessian_row_summary.csv"), rows_summary)
    write_rows_txt(os.path.join(output_root, "train_test_rect_hessian_row_summary.txt"), rows_summary)
    ...
    final_payload: Dict[str, object] = {
        "meta": {
            ...
            "epochs": epochs,
            "methods": methods,
            "formula_T": "T[A,B] = g_train(A)^T H_A g_test(B)",
            "formula_C": "C[A,B] = T[A,B] / sqrt(S_train[A] * S_test[A,B])",
            "S_train": "g_train(A)^T H_A g_train(A)",
            "S_test": "g_test(B)^T H_A g_test(B)",
            "hessian_engine": "empirical_ggt_no_damping",
    ...
    final_json = os.path.join(output_root, "train_test_rect_hessian_summary.json")
    final_txt = os.path.join(output_root, "train_test_rect_hessian_summary.txt")
